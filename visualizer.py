...
def ensure_dependencies():
    ...
    try:
        ...
        result = subprocess.run(
            ["uv", "pip", "install", "flask", "pandas"], check=True, capture_output=True, text=True
        )
        print(result.stdout)
        return True
...
def find_data_files(data_dir="data"):
    ...
    all_files = sorted(csv_files + json_files, key=lambda x: os.path.getmtime(x), reverse=True)
    ...
    for col in numeric_columns:
        if col in df.columns:
            ...
            if before_count != after_count:
                logger.warning(
                    f"Converted {before_count-after_count} non-numeric values to NaN in '{col}'"
                )
...
def get_chart_data(df):
    ...
    metrics = {
        "co2": {"color": "rgb(255, 99, 132)", "label": "CO2 (ppm)", "unit": "ppm"},
        "temperature": {"color": "rgb(255, 159, 64)", "label": "Temperature (°C)", "unit": "°C"},
        "humidity": {"color": "rgb(54, 162, 235)", "label": "Humidity (%)", "unit": "%"},
        "pressure": {"color": "rgb(75, 192, 192)", "label": "Pressure (hPa)", "unit": "hPa"},
    }
    ...
    for metric_key, metric_info in metrics.items():
        if metric_key in df.columns:
            ...
            dataset = {
                ...
                "backgroundColor": (
                    metric_info["color"].replace("rgb", "rgba").replace(")", ", 0.1)")
                ),
                "fill": True,
                "tension": 0.4,
            ...
            chart_data[metric_key] = {"datasets": [dataset], "metric_info": metric_info}
...
def get_data():
    ...
    try:
        ...
        logger.debug("Chart data created")
...
def main():
    ...
    parser.add_argument(
        "--port", "-p", help="Port to run the server on (default: 5000)", type=int, default=5000
    )
    parser.add_argument(
        "--host", help="Host to run the server on (default: 127.0.0.1)", default="127.0.0.1"
    )
    parser.add_argument(
        "--debug", help="Enable debug mode with verbose logging", action="store_true"
    )
    parser.add_argument(
        "--install", help="Install dependencies before running", action="store_true"
    )
    ...
    print("Starting Aranet4 Data Visualizer")
