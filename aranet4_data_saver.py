...
class Aranet4DataSaver:
    ...
    def get_current_readings(self) -> Dict[str, Any]:
        ...
        try:
            ...
            readings = {"timestamp": datetime.datetime.now().isoformat(), "device_mac": device_mac}
    ...
    def _save_as_csv(self, filepath: str):
        ...
        for reading in self.data_buffer:
            fieldnames_set.update(reading.keys())
        ...
        if "timestamp" in fieldnames:
            ...
            timestamp_fieldnames = ["timestamp"]
        fieldnames = timestamp_fieldnames + fieldnames
...
def interactive_config(config_path: str):
    ...
    aranet4 = importlib.import_module("aranet4")
    ...
    config: Dict[str, Any] = {
        ...
        "data_collection": {
            ...
            "collect": {"co2": True, "temperature": True, "humidity": True, "pressure": True},
            "buffer_size": 10,
        },
        "storage": {
            "data_dir": "../data",
    ...
    def on_device_found(advertisement: Any) -> None:
        if (
            advertisement.device
            and advertisement.device.name
            and "Aranet" in advertisement.device.name
        ):
            devices.append(
                {
    ...
    while True:
        try:
            default_interval = config["data_collection"]["polling_interval"]
            prompt = f"\nEnter polling interval in seconds [default: {default_interval}]: "
            interval_str = input(prompt)
            if interval_str.strip():
    ...
    while True:
        try:
            log_choice = input("Choose logging level [default: INFO]: ")
...
def main():
    ...
    parser.add_argument(
        "--install", "-i", action="store_true", help="Install dependencies before running"
    )
    parser.add_argument(
        "--historical",
        "--historical-only",
    ...
    parser.add_argument(
        "--configure", "-C", action="store_true", help="Run interactive configuration wizard"
    )
    ...
    if args.configure:
        try:
            # Check if aranet4 module is available
            import importlib.util

            if importlib.util.find_spec("aranet4") is None:
                raise ImportError("aranet4 not found")
        except ImportError:
    ...
    try:
        # Use importlib to check if aranet4 is available
        import importlib.util
